...
class XYGraph {
	...
	xAxisRange : NumberRange = {start : -5, end : 5};
	yAxisRange : NumberRange = {start : -5, end : 5};
	axisLineWidth : number = 2;
	axisHeadMargin : number = 5;
	color : string = Constants.Colors.Black;

	f : (x : number) => number;

	constructor(layout : Layout, f : (x : number) => number) {
		this.layout = layout;
		this.f = f;
	}
	...
	private renderArrow(
		...
		ctx.lineTo(
			from.x-headlen*Math.cos(oAngle-Math.PI/7),
			from.y-headlen*Math.sin(oAngle-Math.PI/7)
		);
	...
	render(ctx : CanvasRenderingContext2D, cp : ContentProvider) {
		...
		ctx.beginPath();
		let lastJ = 0;
		for (let i = this.layout.computed.position.x;
